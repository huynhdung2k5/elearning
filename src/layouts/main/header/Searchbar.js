...
const APPBAR_MOBILE = 64; // kích cỡ appbar trên mobile
const APPBAR_DESKTOP = 92; // kích cỡ appbar trên desktop
...
function Searchbar() {
  const { push, pathname } = useRouter(); // routers

  const { translate } = useLocales(); // locales

  const [open, setOpen] = useState(false); // state đóng mở thanh tìm kiếm

  const [searchQuery, setSearchQuery] = useState(''); // query tìm kiếm

  const reduceItems = NavConfig.map((list) =>
    handleLoop(list.items, translate(list.subheader))
  ).flat(); // lọc kết quả tìm kiếm
